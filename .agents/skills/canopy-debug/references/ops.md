...
Read `assets/policies/debug-output.md` for banner format, phase registry, and phase descriptions.
...
* EXECUTE_WITH_TRACE << target_skill | skill_args
  * locate target_skill at `<skills-root>/<target_skill>/SKILL.md` (skills-root is `.claude/skills/` on Claude Code, `.github/skills/` on Copilot; SKILL.md is uppercase per agentskills.io spec)
...
Read `assets/policies/debug-output.md` for state symbols and trace file format.
...
Read `assets/policies/debug-output.md` for simulation format and mode behavior rules.
