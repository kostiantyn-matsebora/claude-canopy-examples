...
2. Analyze content. For every distinct block of content, apply the **Category Decision Flowchart** from `assets/policies/category-decision-flowchart.md` (in order — use the first matching test). Additionally map:
   ...
   - Inline ops with a stable `<<`/`>>` signature whose outputs feed downstream consumers (v0.22.0+) → declare `> **Input contract:** assets/schemas/<op>-input.json` and `> **Output contract:** assets/schemas/<op>-output.json` blockquotes on each op definition; create the schema files alongside the op. Default scaffold: `type: object`, `additionalProperties: true`, one `properties.<name>` of `type: string` per `<<` / `>>` named field. Refine the field types where the source skill suggests stricter types (booleans, numbers, enums).
